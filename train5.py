...
from loss_helper.SaliencyGuidedLossV3 import SaliencyGuidedLossV3
...
def train(epoch: int, end_epoch: int, batchWiseAug, model, loader, criterion, optimizer, device, num_classes):
    ...
    for inputs, masks, targets, has_masks in tqdm(loader, total=len(loader), desc="Training epoch [{0}/{1}]".
        ...
        total_loss, cls_loss, saliency_loss1, saliency_loss2 = criterion(outputs, targets, feature_maps, masks, has_masks, epoch) #SaliencyGuideLoss trả về 4 tham số
    

        total_loss.backward()
        optimizer.step()
        metrics.update_train(cls_loss=cls_loss,
                             saliency_loss1=saliency_loss1,
                             saliency_loss2=saliency_loss2,
                             has_masks=has_masks, 
                             outputs=outputs, 
                             targets=targets, 
...
def main():
    ...
    train_criterion = SaliencyGuidedLossV3(type="train", 
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        if save_metrics:
            metric_row = {
                ...
                "train_bce_loss" if saliency_type == 1 else "train_focal_loss": train_metrics.avg_saliency_loss1,
                "train_dice_loss" if saliency_type == 1 else "train_tversky_loss": train_metrics.avg_saliency_loss2,

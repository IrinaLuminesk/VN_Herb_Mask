...
class FMixHierSaliency(Transform):
    ...
    def forward(self, images: torch.Tensor, masks: torch.Tensor, labels: torch.Tensor):
        ...
        fourrier_mask = params["fourrier_mask"]
        lam = params["lam"]
        mixed_images = self.transform_images(images=images, mask=fourrier_mask)
        mixed_masks = self.transform_masks(masks=masks, binary_mask=fourrier_mask)
        mixed_labels = self.transform_labels(labels=labels, lam=lam)
        has_masks = mixed_masks.any(dim=(1, 2, 3))
        return mixed_images, mixed_masks, mixed_labels, has_masks
    ...
    def transform_images(self, images: torch.Tensor, mask: torch.Tensor):
        ...
        return mixed_img
    def transform_masks(self, masks: torch.Tensor, binary_mask: torch.Tensor):
        original_mask = masks
        donor_mask = masks.roll(1,0)
        mixed_masks = original_mask * binary_mask + donor_mask * (1 - binary_mask)
        return mixed_masks
    def transform_labels(self, labels: torch.Tensor, lam: torch.Tensor):
        ...
        if labels.ndim == 2:
            ...
            for idx, key in enumerate(self.number_of_class.keys()):
                labels_onehot = F.one_hot(labels[:, idx], num_classes=self.number_of_class[key]).float()

                donor_labels_onehot = labels_onehot.roll(1, 0)

                mixed_labels = lam * labels_onehot + (1 - lam) * donor_labels_onehot
                new_labels[key] = mixed_labels
        return new_labels

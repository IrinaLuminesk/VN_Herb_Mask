...
from model_builder.hierarchy_model.GINN import GINN
...
from loss_helper.HierarchyLoss import HierarchyLoss
...
def validate(epoch, end_epoch, model, loader, criterion, device, num_classes):
    ...
    with torch.no_grad():
        for inputs, targets in tqdm(loader, total=len(loader), desc="Validating epoch [{0}/{1}]".
            ...
            outputs = model(inputs)
            loss = criterion(outputs, targets)
            metrics.update_test(loss=loss, outputs=outputs[0], targets=targets[:, 0], type="hard")
    return metrics
...
def main():
    ...
    train_data = DatasetLoader(
        ...
        hierarchy_columns= ["Species", "Genus", "Family", "Order"],
        std=std,
        mean=mean,
        img_size=img_size,
